...
export default class Live extends Plugin {
	...
	async onload() {
		...
		const code = `async function() {
			...
			await app.plugins.disablePlugin("lorehold");
			await app.plugins.enablePlugin("lorehold");
		}`;
		(this.app as any).reloadRelay = new Function("return " + code);

		this.addCommand({
			id: "reload",
			name: "Reload Lorehold",
			callback: (this.app as any).reloadRelay(),
		});

		// lorehold: smoke-test login. In production this would be a proper
		// form in the settings tab; for now a modal prompt is enough.
		this.addCommand({
			id: "lorehold-login-password",
			name: "Log in with email + password",
			callback: () => {
				const endpoints = this.loginManager.getEndpointManager();
				const existingUrl = endpoints.getServerUrl() || "";
				new LoreholdPasswordModal(
					this.app,
					existingUrl,
					async (identity, password, serverUrl) => {
						try {
							if (serverUrl !== existingUrl) {
								await endpoints.setServerUrl(serverUrl);
								// Re-create the PocketBase client against the new URL
								// before the login attempt.
								await this.loginManager.reload();
							}
							await this.loginManager.loginWithPassword(identity, password);
							new Notice("Lorehold: logged in");
						} catch (e: any) {
							new Notice(`Lorehold login failed: ${e?.message || e}`);
						}
					},
				).open();
			},
		...
		this.addCommand({
			id: "lorehold-set-server-url",
			name: "Set server URL",
			callback: () => {
				const endpoints = this.loginManager.getEndpointManager();
				const existingUrl = endpoints.getServerUrl() || "";
				new LoreholdServerUrlModal(this.app, existingUrl, async (url) => {
					try {
						await endpoints.setServerUrl(url);
						await this.loginManager.reload();
						new Notice(url ? `Lorehold: server URL set to ${url}` : "Lorehold: server URL cleared");
					} catch (e: any) {
						new Notice(`Lorehold: failed to set server URL: ${e?.message || e}`);
					}
				}).open();
			},
		});
...
class LoreholdPasswordModal extends Modal {
	private identity = "";
	private password = "";
	...
	onOpen(): void {
		...
		contentEl.createEl("h2", { text: "Log in to Lorehold" });

		new Setting(contentEl)
			.setName("Server URL")
			.setDesc(
				"Your Lorehold server URL. Use the Tailscale view of your home server, e.g. http://100.64.0.5:8090. Leave blank to use the build-time default.",
			)
			.addText((t) =>
				t
					.setPlaceholder("http://100.x.y.z:8090")
		...
		new Setting(contentEl)
			...
			.addText((t) =>
				t
					...
					.onChange((v) => (this.identity = v.trim())),
			);
		...
		new Setting(contentEl).addButton((b) =>
			b
				...
				.onClick(async () => {
					...
					this.close();
					await this.onSubmit(this.identity, this.password, this.serverUrl);
				}),
		);
	}
...
class LoreholdServerUrlModal extends Modal {
	private url: string;
	...
	onOpen(): void {
		const { contentEl } = this;
		contentEl.createEl("h2", { text: "Lorehold server URL" });
		contentEl.createEl("p", {
			text:
				"Enter the URL of your Lorehold server. With Tailscale node sharing, this is the IP / hostname your device sees for the home server (e.g. http://100.64.0.5:8090). Leave blank to clear and fall back to the build-time default.",
		});

		new Setting(contentEl).setName("Server URL").addText((t) =>
			t
				.setPlaceholder("http://100.x.y.z:8090")
				.setValue(this.url)
