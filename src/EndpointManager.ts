...
export interface EndpointSettings {
	...
	// lorehold: per-user override for the self-hosted server URL. Each player
	// enters their own Tailscale view of the home server (the IP / hostname
	// can differ across users when the server is shared via Tailscale node
	// sharing). When set, used as both apiUrl and authUrl. Falls back to the
	// compile-time API_URL / AUTH_URL defines if empty.
	serverUrl?: string;
}
...
export class EndpointManager {
	 ...
	 */
	getApiUrl(): string {
		const overrideUrl = this.settings?.get()?.serverUrl?.trim();
	...
	/**
	 * lorehold: persist the per-user server URL to settings. Both apiUrl and
	 * authUrl resolve to this value. Empty string clears the override.
	 */
	async setServerUrl(url: string): Promise<void> {
		const trimmed = url.trim();
		if (!this.settings) {
			throw new Error("EndpointManager: settings not initialized");
		}
		await this.settings.update((current) => ({
			...current,
			serverUrl: trimmed || undefined,
		}));
	}

	getServerUrl(): string | undefined {
		return this.settings?.get()?.serverUrl?.trim() || undefined;
	}
	...
	/**
	 * lorehold: upstream Relay validates enterprise tenants via a license
	 * JWT fetched from /.well-known/relay.md/license and signed by Relay's
	 * RSA key. Self-hosted lorehold has no such license, so we short-circuit and
	 * always fall back to the compile-time API_URL / AUTH_URL defines.
	 */
	async validateAndSetEndpoints(_timeoutMs: number = 10000): Promise<{
		success: boolean;
		error?: string;
