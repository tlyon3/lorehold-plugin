...
export class LoginManager extends Observable<LoginManager> {
	...
	/**
	 * lorehold: reload the PocketBase client against the current authUrl from
	 * EndpointManager. Used after the user updates the server URL setting.
	 */
	async reload(): Promise<void> {
		this.recreatePb();
	}

	private recreatePb(): void {
		const pbLog = curryLog("[Pocketbase]", "debug");
		this.pb = new PocketBase(this.endpointManager.getAuthUrl(), this.authStore);
		this.pb.beforeSend = (url, options) => {
			pbLog(url, options);
	...
	/**
	 * lorehold: primary auth path. PocketBase's built-in email+password.
	 * Avoids OAuth2 entirely so we don't need to host identity providers.
	 */
	async loginWithPassword(identity: string, password: string): Promise<boolean> {
		this.beforeLogin();
